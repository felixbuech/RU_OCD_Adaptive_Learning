function taskData = al_helicopterLoop(taskParam, condition, taskData, trial, file_name_suffix)
%AL_HELICOPTERLOOP This function runs the cannon-task trials for the Leipzig Helicopter version
%
%   Input
...
elseif isequal(taskParam.gParam.saveName, 'asymmetric')
    ...
    concentration = unique(taskData.concentration);
    taskData.savename = sprintf('Helicopter_%s_g%d_conc%d_%s%s', taskParam.trialflow.exp, taskParam.subject.group, concentration, taskParam.subject.ID, file_name_suffix);
end
...
% Give Feedback 
% -----------------------------
if ~isequal(taskParam.trialflow.reward, 'asymmetric') && ~taskParam.gParam.passiveViewing
    currPoints = nansum(taskData.hit);

% Compute total number of valid trials (ignores NaNs)
totalTrials = sum(~isnan(taskData.hit)); 

% Compute hit ratio dynamically
...
if isequal(taskParam.gParam.language, 'German')
    if hitRatio > 0.5
        ...
        txt = sprintf(['Sie haben %.0f Einheiten lebensrettende Medikamente gesichert.\n\n ' ...
            'Versuchen Sie, noch mehr zu fangen, oder mehr Menschen werden sterben!'], currPoints);
    end
...
elseif isequal(taskParam.trialflow.reward, 'asymmetric') && ~taskParam.gParam.passiveViewing
...
% --- Present Feedback Screen with Text and Picture ---
feedback = true;
al_bigScreen(taskParam, header, txt, feedback);

% --- Additional Eye Tracking Feedback (if applicable) ---
if taskParam.gParam.eyeTracker && taskParam.gParam.onlineSaccades
    feedback = true;
    if isequal(taskParam.gParam.language, 'German')
    ...
    elseif isequal(taskParam.gParam.language, 'English')
        header = 'Eye Movement Information';
        txt = sprintf('You looked away %d times during this block.', sum(taskData.sacc));
    else
        error('Language parameter unknown');
    end
    al_bigScreen(taskParam, header, txt, feedback);
...
% --- Wait until keys are released to prevent accidental skips ---
KbReleaseWait();
