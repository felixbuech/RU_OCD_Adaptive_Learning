...
MAINPATH = 'C:\Users\pc\Desktop\Behavioral_EyeLink_Data\resting_state';
cd(MAINPATH)
PATHOUT = fullfile(MAINPATH, filesep);
addpath(MAINPATH);
...
%Only eyes open
   ...
   % EEG trigger
   if EEG == 1
   sendTrigger(ioObj,lptAddr,trigDur,trigger_open);
   end
   if ET == 1
...
function sendTrigger(ioObj,lptAddr,duration,trigger)
    io64(ioObj, lptAddr, uint8(trigger));   % set code
    WaitSecs(duration);
    io64(ioObj, lptAddr, uint8(0));         % reset to 0
end
